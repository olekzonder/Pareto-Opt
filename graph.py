...
def plot_graph(points=False, unopt=False, pareto=False, optimal=False,most_optimal=False,ideal = False):
    ...
    if most_optimal:
        fig.add_trace(go.Scatter(x=[point.f1 for point in most_optimal],y=[point.f2 for point in most_optimal], marker=dict(color='darkblue') mode='markers',name='Наиболее оптимальный проект'))
    if ideal:
